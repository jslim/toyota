...
type PrivacyPolicyPageData = FilteredEntity<LegalPageContentType>;

export interface PrivacyPolicyPageProps extends PageProps {
  data: PrivacyPolicyPageData;
}

const PrivacyPolicy: FC<PrivacyPolicyPageProps> = ({ data }) => {
  ...
  return (
    <main className={classNames('PrivacyPolicy')}>
...
export const getStaticProps: GetStaticProps<PrivacyPolicyPageProps> = async ({ params }) => {
  ...
  const data = await apiContentful.getEntryBySlug('privacy-policy', 'legalPage', { locale, include: 10 });

  return {
    props: {
      head: { title: data?.fields?.pageTitle ?? 'Privacy Policy' },
